#!/usr/bin/env node
// MCP dash-f2f — leitura do banco do painel: sites cadastrados e histórico de
// varreduras de plugins WordPress, sempre escopados a um usuário.
//
// Somente leitura em três camadas:
//  1. a role do banco (`dash_f2f_reader`) só tem GRANT SELECT;
//  2. todas as consultas são SQL fixo e parametrizado (não há tool de query livre);
//  3. as tools são anotadas como readOnlyHint no protocolo.

import { McpServer } from '@modelcontextprotocol/sdk/server/mcp.js';
import { StdioServerTransport } from '@modelcontextprotocol/sdk/server/stdio.js';
import { z } from 'zod';

import { resolveOwnerId } from './db';
import * as q from './queries';
import { diffScans, CHANGE_LABEL } from '../../src/lib/diff';
import { normalizeSiteUrl, displayUrl } from '../../src/lib/site-url';
...
/** O driver devolve timestamptz como Date; no texto queremos ISO, não o formato longo. */
...
async function main() {
  ...
  // Resolve o dono uma única vez, no boot. Nenhuma tool aceita owner por
  // parâmetro — o escopo não é negociável em runtime.
  ...
  const server = new McpServer(
    ...
    {
      instructions:
        `Leitura do painel dash-f2f para a conta ${email}. Expõe os sites WordPress ` +
  ...
  /** Resolve uma URL informada pelo agente para um site do usuário. */
